...
class StoreQueue(implicit p: Parameters) extends XSModule
  ...
  gpaddrModule.io.raddr(StorePipelineWidth) := (cmtPtrExt(0) + commitCount).value
  /**
    * Enqueue at dispatch
    *
  ...
  io.exceptionAddr.gpaddr := gpaddrModule.io.rdata(StorePipelineWidth)
